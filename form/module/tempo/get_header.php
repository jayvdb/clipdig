<?php
	$media = "tempo"; //CHANGE THIS


	$search_link ="";$link ="";$title ="";$time="";
	for($i=$start;$i<=$to;$i++){
		$search_link .= '||'.'http://search.tempo.co/index.php/search/result?type=semua&fld=news&vSearch='.$search_.'&page='.$i; //CHANGE THIS
	}
	...
	for($j=0;$j<count($pecah_search_link);$j++){
		...
		foreach($html->find('div#kiri-680') as $list){
			...
			foreach($list->find('div.block-news h4 a') as $ti){ // find title
				$title_ = $ti->plaintext;
				$title[$j] .='||'.$title_;
			}
			foreach($list->find('div.block-news div.url') as $tim){ //find time
				$time_ = $tim->plaintext;
				$time_ =explode(" ",$time_);
