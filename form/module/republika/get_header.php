<?php
	$media = "republika"; //CHANGE THIS
	
	
	$search_link ="";$link ="";$title ="";$time="";
	for($i=$start;$i<=$to;$i++){
		$search_link .= '||'.'http://www.republika.co.id/indeks/hot_topic/'.$search__.'/'.(($i-1)*15); //CHANGE THIS
	} 
	...
	for($j=0;$j<count($pecah_search_link);$j++){
		...
		foreach($html->find('div.left-kanal') as $list){ 
			...
			foreach($list->find('h4.jdl-ib') as $ti){ // find title
				$title_ = $ti->plaintext;
				$title[$j] .='||'.$title_;
			}
