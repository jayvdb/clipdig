<?php
	$media = "okezone"; //CHANGE THIS
	
	
	$search_link ="";$link ="";$title ="";$time="";
	for($i=$start;$i<=$to;$i++){
		$page= ($i-1)*12;
		$search_link .= '||'.'http://search.okezone.com/search/?q='.$search__.'&type=1&highlight=1&sort=desc&start='.$page; //CHANGE THIS
	} 
	...
	for($j=0;$j<count($pecah_search_link);$j++){
		...
		//CHANGE FROM THIS --------------------------------
		foreach($html->find('ul[class=list-n-berita-search list-berita-search]') as $list){ 
			foreach($list->find('h2 a') as $li){ // find link
				$link_ = $li->href;
				$link_ = real_url($link_);
				$link[$j] .='||'.$link_;
			}
			foreach($list->find('h2') as $ti){ // find title
				$title_ = $ti->plaintext;
				$title[$j] .='||'.$title_;
			}
			foreach($list->find('h5') as $tim){ //find time
				$time_ = $tim->plaintext;
				$time_ =explode(" ",$time_);		
