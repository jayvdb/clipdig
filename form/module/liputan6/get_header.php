<?php
	$media = "liputan6"; //CHANGE THIS
	
	
	$search_link ="";$link ="";$title ="";$time="";
	for($i=$start;$i<=$to;$i++){
		$search_link .='||'.'http://www.liputan6.com/search?q='.$search__.'&page='.$i;//CHANGE THIS
	} 
	...
	for($j=0;$j<count($pecah_search_link);$j++){
		...
		foreach($html->find('article.main') as $list){ 
			...
			foreach($list->find('h4') as $ti){ // find title
				$title_ = $ti->plaintext;
				$title[$j] .='||'.$title_;
			}
			foreach($list->find('time.articles--rows--item__time') as $tim){ //find time
				$time_ = $tim->plaintext;
				$time_ =explode(" ",$time_);
