<?php
	$media = "pkspiyungan"; //CHANGE THIS
	
	
	$search_link ="";$link ="";$title ="";$time="";
	for($i=$start;$i<=$to;$i++){
		$search_link .= '||'.'http://www.pkspiyungan.org/search?q='.$search__.'&p='.$i; //CHANG THIS
	} 
	...
	for($j=0;$j<count($pecah_search_link);$j++){
		...
		//CHANGE FROM THIS --------------------------------
		foreach($html->find('div[class=blog-posts]') as $list){ 
			foreach($list->find('h2.entry-title a') as $li){  // find link 
				$link_ = $li->href;						
				$link_ = real_url($link_);						
				$link[$j] .='||'.$link_;
			}
			foreach($list->find('h2.entry-title') as $ti){ // find title
				$title_ = $ti->plaintext;
				$title[$j] .='||'.$title_;
			}
			foreach($list->find('span.time-info') as $tim){ //find time
				$time_ = $tim->plaintext;
				//$time_ = explode("/",$time_);
				//$time_ = UbahXXX(end($time_));
				//$time_ = explode(" ",$time_);
		...
		for($k=0;$k<$banyak;$k++){
			...
			$no=$k+1;
			echo '<br>'.$no.'. <a href="?m=Scrap&l=View&op=edit&kode='.$data_kode.'">'.$data_title.'</a>('.urlencode($data_time).')';
