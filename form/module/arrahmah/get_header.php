<?php
	$media = "arrahmah"; //CHANGE THIS
	
	
	$search_link ="";$link ="";$title ="";$time="";
	for($i=$start;$i<=$to;$i++){
		$search_link .= '||'.'http://www.arrahmah.com/search/'.$search__.'/page/'.$i; //CHANG THIS
	} 
	...
	for($j=0;$j<count($pecah_search_link);$j++){
		...
		foreach($html->find('section.post-items') as $list){ 
			...
			foreach($list->find('h1.headline') as $ti){ // find title
				$title_ = $ti->plaintext;
				$title[$j] .='||'.$title_;
			}
			foreach($list->find('div.item-date') as $tim){ //find time
				$time_ = $tim->plaintext;
				$time_ = explode("/",$time_);
