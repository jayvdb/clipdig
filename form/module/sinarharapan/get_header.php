<?php
	$media = "sinarharapan"; //CHANGE THIS
	
	
	$search_link ="";$link ="";$title ="";$time="";
	for($i=$start;$i<=$to;$i++){
		$search_link .='||'.'http://www.sinarharapan.co/news/cari/'.rawurlencode($search).'/'.(($i-1)*10); //CHANGE THIS
	} 
	...
	for($j=0;$j<count($pecah_search_link);$j++){
		...
		//CHANGE FROM THIS --------------------------------
		foreach($html->find('div.archive') as $list){ 
			foreach($list->find('h3.entry-title a') as $li){  // find link 
				$link_ = $li->href;
				$link_ = real_url($link_);
				$link[$j] .='||'.$link_;
			}
			foreach($list->find('h3.entry-title') as $ti){ // find title
				$title_ = $ti->plaintext;
				$title[$j] .='||'.$title_;
			}
			foreach($list->find('span.date') as $tim){ //find time
				$time_ = $tim->plaintext;
				$time_ = str_replace("%09","",urlencode($time_));
				$time_ = explode("+",$time_);		
