<?php
	$media = "pikiranrakyat"; //CHANGE THIS
	
	
	$search_link ="";$link ="";$title ="";$time="";
	for($i=$start;$i<=$to;$i++){
		$search_link .='||'.'http://www.pikiran-rakyat.com/search/node/'.rawurlencode($search).'?page='.($i-1); //CHANGE THIS
	} 
	...
	for($j=0;$j<count($pecah_search_link);$j++){
		...
		foreach($html->find('ol[class=search-results node-results]') as $list){ 
			...
			foreach($list->find('h3.title') as $ti){ // find title
				$title_ = $ti->plaintext;
				$title[$j] .='||'.$title_;
			}
			foreach($list->find('p.search-info') as $tim){ //find time
				$time_ = $tim->plaintext;
				$time_ = UbahXXX($time_);
				$time_ = explode(" ",$time_);		
